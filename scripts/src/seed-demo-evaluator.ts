...
/**
 * Crea un usuario evaluador DEMO en el emulador de Auth con claims EVALUADOR
 * y acceso a los cursos D y E (puede revisar materiales de ambos).
 * Uso: pnpm seed:demo-evaluator
 */
async function main(): Promise<void> {
  const flags = parseFlags(process.argv.slice(2));
  requireEmulator(false);

  const email = flags.email ?? "evaluador@demo.cl";
  const password = flags.password ?? "Demo1234";
  const displayName = flags.name ?? "Evaluadora Demo";
  ...
  console.log("Claims asignados: role=EVALUADOR, courses=[D, E]");
