...
- **Todo se decora**: además de animalitos y decoraciones, ahora **herramientas y talismanes también se colocan** en el mapa; cada objeto aporta bonus **según su categoría** — decoración +2% 🪙, animalitos/ayudantes +1% 🪙 y +1% ⭐, herramientas +2% ⚡ y talismanes +2% ⭐ (tope +20% por tipo), visible en "Mejoras activas". Server-authoritative (`saveFarmLayout`).
- **Casa por dentro**: **6 salas temáticas** (Living, Cocina, Dormitorio, Estudio, **Taller**, Terraza) desbloqueadas por nivel, cada una con **muebles propios**, y los **colores del piso cambian según la casa** elegida.
- **Más formas de casa**: **10 estilos** (🏕️🛖🏠🏡🏘️🏰🏚️🏯🏛️🏭) elegibles y **entrar a la casa**.
- **Usabilidad del mapa**: la escena ocupa todo el ancho y la bandeja **"Decora tu granja" va debajo** (ya no tapa el mapa); botón **"Ocultar/Mostrar cultivos"** que **colapsa la parcela** para dejar el mapa libre y colocar adornos (**se oculta solo al empezar a colocar un objeto**); botón **"＋ Ampliar parcela"**; panel de instrucciones "¿Cómo juego en la granja?" y botón **Tienda** arriba.
- **Capturas**: granja (`docs/assets/granja-ciudadana.png`), mapa libre para adornos (`docs/assets/granja-adornos.png`) e interior (`docs/assets/granja-casa.png`); script `scripts/screenshot-farm.mjs`.
