...
- **Los objetos colocados dan mejora** (server-authoritative, `saveFarmLayout`): cada objeto aporta bonus **según su categoría** — decoración +2% 🪙, animalitos/ayudantes +1% 🪙 y +1% ⭐, herramientas +2% ⚡ y talismanes +2% ⭐ (tope +20% por tipo), visible en "Mejoras activas".
- **Casa por dentro**: cada **piso** (planta baja, primer piso, segundo piso y terraza, por nivel) es una sala distinta con **muebles propios** (living, dormitorio, estudio, terraza).
